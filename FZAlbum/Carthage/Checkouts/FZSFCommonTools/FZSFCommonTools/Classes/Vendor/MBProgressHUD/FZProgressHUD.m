...
#import "FZProgressHUD.h"
#import <tgmath.h>


#ifndef kCFCoreFoundationVersionNuFZer_iOS_7_0
    #define kCFCoreFoundationVersionNuFZer_iOS_7_0 847.20
#endif

#ifndef kCFCoreFoundationVersionNuFZer_iOS_8_0
    #define kCFCoreFoundationVersionNuFZer_iOS_8_0 1129.15
#endif

#define FZMainThreadAssert() NSAssert([NSThread isMainThread], @"FZProgressHUD needs to be accessed on the main thread.");

CGFloat const FZProgressMaxOffset = 1000000.f;

static const CGFloat FZDefaultPadding = 4.f;
static const CGFloat FZDefaultLabelFontSize = 16.f;
static const CGFloat FZDefaultDetailsLabelFontSize = 12.f;


@interface FZProgressHUD () {
    // Deprecated
...
@interface FZProgressHUDRoundedButton : UIButton
@end


@implementation FZProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    FZProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    FZProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (FZProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            FZProgressHUD *hud = (FZProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = FZProgressHUDAnimationFade;
    _mode = FZProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
    _defaultMotionEffectsEnabled = YES;

    // Default color, depending on the current iOS version
    BOOL isLegacy = kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNuFZer_iOS_7_0;
...
- (void)animateIn:(BOOL)animatingIn withType:(FZProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == FZProgressHUDAnimationZoom) {
        type = animatingIn ? FZProgressHUDAnimationZoomIn : FZProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == FZProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == FZProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == FZProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == FZProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
#pragma clang diagnostic pop
    ...
    if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuFZer_iOS_7_0) {
...
- (void)done {
    ...
    FZProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<FZProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    FZBackgroundView *backgroundView = [[FZBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = FZProgressHUDBackgroundStyleSolidColor;
    ...
    FZBackgroundView *bezelView = [FZBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:FZDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:FZDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [FZProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:FZDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[FZRoundProgressView class]];

    FZProgressHUDMode mode = self.mode;
    if (mode == FZProgressHUDModeIndeterminate) {
    ...
    else if (mode == FZProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[FZBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == FZProgressHUDModeDeterminate || mode == FZProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[FZRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == FZProgressHUDModeAnnularDeterminate) {
            [(FZRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == FZProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == FZProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[FZProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[FZProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[FZRoundProgressView class]]) {
        FZRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [FZRoundProgressView appearanceWhenContainedIn:[FZProgressHUD class], nil];
#else
        appearance = [FZRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[FZProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((FZRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((FZRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[FZBarProgressView class]]) {
        FZBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [FZBarProgressView appearanceWhenContainedIn:[FZProgressHUD class], nil];
#else
        appearance = [FZBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[FZProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((FZBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((FZBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    FZBackgroundView *bezelView = self.bezelView;
...
- (void)layoutSubviews {
    ...
    // would trigger a zoFZie object access.
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? FZDefaultPadding : 0.f;
...
- (void)setMode:(FZProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == FZProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
- (void)updateForCurrentOrientationAnimated:(BOOL)animated {
    ...
    BOOL iOS8OrLater = kCFCoreFoundationVersionNuFZer >= kCFCoreFoundationVersionNuFZer_iOS_8_0;
...
@implementation FZRoundProgressView
...
- (void)drawRect:(CGRect)rect {
    ...
    BOOL isPreiOS7 = kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNuFZer_iOS_7_0;
...
@implementation FZBarProgressView
...
@implementation FZBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuFZer_iOS_7_0) {
            _style = FZProgressHUDBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
            _blurEffectStyle = UIBlurEffectStyleLight;
#endif
            if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuFZer_iOS_8_0) {
                _color = [UIColor colorWithWhite:0.8f alpha:0.6f];
            } else {
                _color = [UIColor colorWithWhite:0.95f alpha:0.6f];
            }
        } else {
            _style = FZProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(FZProgressHUDBackgroundStyle)style {
    if (style == FZProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNuFZer_iOS_7_0) {
        style = FZProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    FZProgressHUDBackgroundStyle style = self.style;
    if (style == FZProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuFZer_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == FZProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuFZer_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation FZProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [FZProgressHUD allHUDsForView:view];
    for (FZProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable FZProgressHUDCompletionBlock)completion {
...
- (BOOL)diFZackground {
    FZBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == FZProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDiFZackground:(BOOL)diFZackground {
    FZMainThreadAssert();
    self.backgroundView.style = FZProgressHUDBackgroundStyleSolidColor;
    self.backgroundView.color = diFZackground ? [UIColor colorWithWhite:0.f alpha:.2f] : [UIColor clearColor];
}
...
@implementation FZProgressHUDRoundedButton
